...
impl Level {
    ...
    pub fn calculate_earned_exp(
        ...
        stamina: &Stamina,
        lazy_meter: &LazyMeter,
    ) -> i64 {
        let stamina_bonus = 100.0 * (0.25 * (stamina.stamina_val as f64 / MAX_STAT_VALUE as f64));
        let lazy_penalty = user_defined_exp * 0.5 * (lazy_meter.lazy_meter_val as f64 / MAX_STAT_VALUE as f64);
        
        let final_exp = (user_defined_exp + stamina_bonus) - lazy_penalty;
...
#[derive(Serialize, Deserialize, Debug, Default, Clone)]
pub struct LazyMeter {
    pub lazy_meter_val: i32,
}

impl LazyMeter {
    /// Resets the lazy meter to its minimum value.
    pub fn reset_lazy_meter(&mut self) {
        self.lazy_meter_val = 0;
    }

    /// Increases the lazy meter based on the time overage during a break.
    pub fn increase_lazy_meter(&mut self, time_extra: f64, time_break: f64) {
        if self.lazy_meter_val == MAX_STAT_VALUE {
            return;
        }
        ...
        self.lazy_meter_val = self.lazy_meter_val.saturating_add(increase);
        if self.lazy_meter_val > MAX_STAT_VALUE {
            self.lazy_meter_val = MAX_STAT_VALUE;
        }
    }
}
...
impl fmt::Display for Player {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        writeln!(f, "  Lazy Meter: {}", self.lazy_meter.lazy_meter_val)?;
